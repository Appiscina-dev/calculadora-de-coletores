...
whatsappForm.addEventListener('submit', async function(e) {
    ...
    window.open(`https://wa.me/554398047951?text=${encodeURIComponent(whatsappMsg)}`, '_blank');
...
logoLink.addEventListener('click', function(e) {
    e.preventDefault();
    window.open('https://appiscina.com.br', '_blank');
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const unwantedBrands = ['komeco', 'solis', 'mastersol', 'soria'];
    
    unwantedBrands.forEach(brand => {
        const option = brandSelect.querySelector(`option[value="${brand}"]`);
        if (option) {
            brandSelect.removeChild(option);
        }
    });
});
